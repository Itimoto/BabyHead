...
void loop() {
    ...
    panAngle += 90;               //Bring the Pan/Tilt into a Servo's working range
    tiltAngle += 90;
    
    if(panAngle > 0 && panAngle < 180){ //Keeps the Servo within its bounds
...
void readMPU(){
    ...
    lastAccelX = AccAngleX;
    lastAccelY = AccAngleY;
    ...
    GyroX = (GyroX * .96) + ((AccAngleX - lastAccelX)/dT) * .4;
    GyroY = (GyroY * .96) + ((AccAngleY - lastAccelY)/dT) * .4;
